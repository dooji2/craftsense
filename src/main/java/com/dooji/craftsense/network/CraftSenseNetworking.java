...
public class CraftSenseNetworking {
    public static void init() {
        ...
        ServerPlayNetworking.registerGlobalReceiver(CraftItemPayload.ID, (payload, context) -> {
            context.server().execute(() -> {
                handleCraftItemPayload(payload.getRecipeId(), context.player());
            });
        });
    ...
    private static void handleCraftItemPayload(String recipeIdStr, ServerPlayerEntity player) {
        RecipeManager recipeManager = player.getServer().getRecipeManager();
        ...
        if (recipeEntryOptional.isPresent()) {
            ...
            if (recipe instanceof CraftingRecipe craftingRecipe) {
                ...
                if (player.currentScreenHandler instanceof CraftingScreenHandler handler) {
                    ...
                    if (hasAllIngredients(inventory, gridInventory, craftingRecipe)) {
                        consumeIngredients(craftingRecipe, gridInventory, inventory);
                        player.networkHandler.sendPacket(createSlotUpdatePacket(handler));
    ...
    private static Packet<?> createSlotUpdatePacket(CraftingScreenHandler handler) {
        return new ScreenHandlerSlotUpdateS2CPacket(handler.syncId, -1, 0, handler.getCursorStack());
    }
