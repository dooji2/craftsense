...
public class CraftSenseStatsScreen extends Screen {
    ...
    public void render(MatrixStack matrices, int mouseX, int mouseY, float delta) {
        ...
        DrawableHelper.drawCenteredTextWithShadow(matrices, this.textRenderer, this.title.asOrderedText(), this.width / 2, 15, 0xFFFFFF);
        ...
        renderPieChart(matrices, mouseX, mouseY, areaCenterY);
        renderLegend(matrices, areaCenterY);

        if (categoryTotals.isEmpty()) {
            DrawableHelper.drawCenteredTextWithShadow(matrices, this.textRenderer, Text.of(Text.translatable("screen.craftsense.no_history").getString()).asOrderedText(), this.width / 2, areaCenterY, 0xAAAAAA);
        }
    }

    private void renderPieChart(MatrixStack matrices, int mouseX, int mouseY, int areaCenterY) {
        ...
        RenderSystem.setShaderTexture(0, pieTextureId);
        DrawableHelper.drawTexture(matrices, cx, drawY, 0, 0, pieTextureWidth, pieTextureHeight, pieTextureWidth, pieTextureHeight);

        int radius = Math.min(100, (pieTextureHeight - 40) / 2);
        int centerX = cx + (radius + 20);
        int centerY = drawY + (radius + 20);
        handlePieHover(matrices, mouseX, mouseY, centerX, centerY, radius);
    }

    private void renderLegend(MatrixStack matrices, int areaCenterY) {
        if (categoryTotals.isEmpty()) {
            return;
        }
        ...
        DrawableHelper.drawCenteredTextWithShadow(matrices, this.textRenderer, Text.of(pageText).asOrderedText(), midX, paginationY + 5, 0xFFFFFF);
        ...
        for (int i = startIndex; i < endIndex; i++) {
            ...
            DrawableHelper.fill(matrices, cx, lineY, cx + 10, lineY + 10, color);
            ...
            DrawableHelper.enableScissor(textStartX, textStartY, textEndX, textEndY);
            renderScrollableText(matrices, this.textRenderer, display, textStartX, textStartY, textEndX, textEndY, 0xFFFFFF);
    ...
    private void generatePieTexture() {
        ...
        transform.multiply(Matrix4f.translate(radius + margin, radius + margin, 0));

        Quaternion rotation = new Quaternion(45.0F, 0.0F, 0.0F, true);
            ...
            for (int px = minX; px <= maxX; px++) {
                ...
                for (int py = minY; py <= maxY; py++) {
                    ...
                    if (dx * dx + dy * dy <= radius * radius) {
                        ...
                        Vector4f pos = new Vector4f(px, py, 0, 1);
    ...
    private void handlePieHover(MatrixStack matrices, int mouseX, int mouseY, int cx, int cy, int radius) {
        float dx = mouseX - cx;
        float dy = mouseY - cy;
        ...
        if (dx * dx + dy * dy <= radius * radius) {
            ...
            if (visibleSlices.size() == 1) {
                ...
                showTooltipForCategory(matrices, category, mouseX, mouseY);
            } else {
                float angle = (float) Math.toDegrees(Math.atan2(dy, dx));
    ...
    private void showTooltipForCategory(MatrixStack matrices, String category, int mouseX, int mouseY) {
    ...
    private static void renderScrollableText(MatrixStack matrices, TextRenderer textRenderer, Text text, int startX, int startY, int endX, int endY, int color) {
        ...
        if (textWidth > availableWidth) {
            ...
            DrawableHelper.drawTextWithShadow(matrices, textRenderer, text, startX - scrollOffset, startY + (endY - startY - 9) / 2, color);
            DrawableHelper.disableScissor();
        } else {
            DrawableHelper.drawTextWithShadow(matrices, textRenderer, text, startX, startY + (endY - startY - 9) / 2, color);
        }
    }
}